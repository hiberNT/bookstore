...
class TestOrderSerializer(TestCase):
    def setUp(self) -> None:
        ...
        )  # Cria um pedido com os dois produtos
        self.order_serializer = OrderSerializer(
            self.order
        )  # serializando a order aqui, Serializar um produto significa transformar um objeto Python

    def test_order_serializer(self):
        serializer_data = (
            self.order_serializer.data
        )  # O .data converte esses dados em um dicionário pronto para ser enviado via API (JSON).
